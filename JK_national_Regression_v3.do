...
************ Date: 2021/06/01, Last edited by JK Won ************************************
*****************************************************************************************
...
corr public_use dist_to_attack dist_to_road


eststo clear
estpost sum public_use dist_to_attack slope_med elev_med dist_to_road dist_Gwangju dist_Daejeon dist_Daegu dist_Busan dist_Seoul ltype_1_1 farmland_perhousehold ///
if sido_nm!="경상남도"&sido_nm!="경상북도"&sido_nm!="제주도"&dist_to_attack<=65
esttab using desc_stats_v3.tex,replace /// 
refcat(public_use "\emph{Dependent Variable}" dist_to_attack "\vspace{0.1em} \\ \emph{Explanatory Variable}" ///
slope_med "\vspace{0.1em} \\ \emph{GIS Controls}" ltype_1_1 "\vspace{0.1em} \\ \emph{Other Controls}",nolabel) cells("count mean sd min max") nostar unstack nonumber ///
...
reg public_use dist_to_attack slope_med elev_med dist_to_road dist_Gwangju dist_Daejeon dist_Daegu dist_Busan dist_Seoul ///
...
areg public_use dist_to_attack slope_med elev_med dist_to_road dist_Gwangju dist_Daejeon dist_Daegu dist_Busan dist_Seoul ///
...
areg public_use dist_to_attack slope_med elev_med dist_to_road dist_Gwangju dist_Daejeon dist_Daegu dist_Busan dist_Seoul ///
ltype_1_1 farmland_perhousehold  ///
if sido_nm!="경상남도"&sido_nm!="경상북도"&sido_nm!="제주도"&dist_to_attack<=65, absorb(sig_cd) vce(cluster sig_cd)
...
esttab m1 m2 m3 m4 using table_3_v3.tex,replace keep(dist_to_attack) b p ///
...
wls0 public_use dist_to_attack slope_med elev_med dist_to_road ///
if sido_nm!="경상남도"&sido_nm!="경상북도"&sido_nm!="제주도"&dist_to_attack<=65, wvars(dist_to_attack dist_to_road) type(abse) noconst robust
estimates store m2
estadd local GIS "Y"
estadd local FE "N"
estadd local CR "N"
...
xi:wls0 public_use dist_to_attack slope_med elev_med dist_to_road  i.sig_cd ///
if sido_nm!="경상남도"&sido_nm!="경상북도"&sido_nm!="제주도"&dist_to_attack<=65, wvars(dist_to_attack dist_to_road) type(abse) noconst robust
estimates store m3
estadd local GIS "Y"
estadd local FE "Y"
...
xi:wls0 public_use dist_to_attack slope_med elev_med dist_to_road i.sig_cd ltype_1_1 farmland_perhousehold   ///
if sido_nm!="경상남도"&sido_nm!="경상북도"&sido_nm!="제주도"&dist_to_attack<=65, wvars(dist_to_attack dist_to_road) type(abse) noconst robust
estimates store m4
estadd local GIS "Y"
estadd local FE "Y"
...
esttab m1 m2 m3 m4 using table_3_wls_v3.tex,replace keep(dist_to_attack) b p ///
nomtitles mgroups("Dependent variable: public_use", pattern(1 0 0 0) ///
span prefix(\multicolumn{@span}{c}{) suffix(})) r2 ///
scalars("GIS GIS controls" "FE County Fixed Effects" "CR Controls")
